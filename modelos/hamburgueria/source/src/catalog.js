...
export function getCatalogUrl() {
    ...
    if (isPrivateHost(window.location.hostname)) {
        ...
        // Quando a landing está aberta pelo Live Server dentro do repositório,
        // usa exatamente a mesma origem e porta do restante da Neoeffex.
        if (pathname.includes("/modelos/hamburgueria/")) {
            return new URL("../../catalogo/" + query, window.location.href).href;
        }

        // No servidor de desenvolvimento do Vite, usa a origem configurada
        // para alcançar o Live Server principal.
        const developmentOrigin = String(catalog.developmentOrigin || "").replace(/\/$/, "");
        return developmentOrigin
            ? developmentOrigin + "/catalogo/" + query
            : new URL("/catalogo/" + query, window.location.origin).href;
    }
