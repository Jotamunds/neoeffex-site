...
Se publicar em uma subpasta, coloque esses itens juntos dentro dela. Não troque caminhos relativos por `/assets/...`, pois isso aponta para a raiz do domínio. Não adicione uma tag `<base>` sem revisar todos os atalhos.
...
Abra a URL final com HTTPS, confirme os arquivos carregados, teste os três atalhos e confira todos os canais de contato. Verifique novamente imagens, preços, mobile e metadados na URL real. Não marque como concluído com base apenas no resultado local.
